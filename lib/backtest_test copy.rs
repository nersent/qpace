...
mod test {
    ...
    use crate::{
        ...
        sym::SymInfo,
        test_utils::{format_fixture_path, ArraySnapshot},
    ...
    impl BacktestTrade {
        pub fn from_trade(trade: &Trade) -> Self {
            return Self {
                ...
                entry_bar_index: trade.entry().map(|x| x.fill_bar_index()),
                entry_price: trade.entry().map(|x| x.price()),
                entry_id: trade.entry().map(|x| x.id().cloned()).unwrap_or(None),
                exit_bar_index: trade.exit().map(|x| x.fill_bar_index()),
                exit_price: trade.exit().map(|x| x.price()),
                exit_id: trade.exit().map(|x| x.id().cloned()).unwrap_or(None),
                profit: Some(trade.pnl()),
                size: Some(trade.size().abs()),
    ...
    impl TestBacktest {
        pub fn new(mut backtest_fixture: BacktestFixture) -> Self {
            ...
            if let Some(fixture_path) = &backtest_fixture.config.fixture {
                ...
                let mut dp: ArcOhlcv = Ohlcv::from_polars(&_df).into();
        ...
        pub fn run(&mut self) {
            ...
            for bar_index in self.ctx.borrow_mut().into_iter() {
                self.strategy.borrow_mut().on_bar_open();
                ...
                if let Some(signal_fixture) = signal_fixture {
                    ...
                    self.strategy.borrow_mut().order(order).unwrap();
                }

                self.strategy.borrow_mut().on_bar_close();

                snapshot_equity.push(self.strategy.borrow().equity());
                snapshot_position_size.push(self.strategy.borrow().position_size());
                snapshot_open_profit.push(self.strategy.borrow().open_profit());
                snapshot_net_profit.push(self.strategy.borrow().net_profit());
                snapshot_open_trades.push(self.strategy.borrow().open_trades().len() as f64);
                snapshot_closed_trades.push(self.strategy.borrow().closed_trades().len() as f64);
                snapshot_gross_loss.push(self.strategy.borrow().gross_loss());
                snapshot_gross_profit.push(self.strategy.borrow().gross_profit());
                snapshot_winning_trades.push(self.strategy.borrow().winning_trades() as f64);
                snapshot_losing_trades.push(self.strategy.borrow().losing_trades() as f64);
            }
            ...
            let closed_trades: Vec<Trade> = strategy.closed_trades().to_vec();
            let open_trades: Vec<Trade> = strategy.open_trades().to_vec();
            let trades: Vec<Trade> = strategy.trades().into_iter().cloned().collect();
            ...
            for trade_fixture in &self.fixture.trades {
                ...
                let trade = trades.iter().find(|t| {
                    t.entry().map(|x| x.id().cloned()).unwrap_or(None) == trade_fixture.entry_id
                        && t.exit().map(|x| x.id().cloned()).unwrap_or(None)
    ...
    pub fn tv_order_contracts() {
        TestBacktest::new(BacktestFixture::load(&format_path(
